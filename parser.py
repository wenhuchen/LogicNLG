...
"""
class ParserRanker(object):
    def __init__(self, parser, vocab, load_from=None):
        self.parser = parser

        with open(vocab) as f:
            self.vocab = json.load(f)

        self.model = Ranker(len(self.vocab), 256, 3, 256, 4)
    ...
    def rank(self, programs, masked_sent, mapping):
        if programs:
            ...
            prog_max_len = 60
            sent_tokens = [self.vocab['[SOS]']] + [self.vocab.get(_, self.vocab['[UNK]']) for _ in masked_sent]
            
            for prog in programs:
                prog_tokens, _  = split_prog(prog)
                for i, elem in enumerate(prog_tokens):
                    if elem in imapping:
                        prog_tokens[i] = imapping[elem]                

                prog_tokens = [self.vocab.get(_, self.vocab['[UNK]']) for _ in prog_tokens]
                prog_tokens = prog_tokens[:prog_max_len] + [self.vocab['[PAD]']] * max(prog_max_len - len(prog_tokens), 0)
                
                labels.append(_)
                progs.append(prog_tokens)
                sents.append(sent_tokens)

            progs = torch.LongTensor(progs).to(self.device)
            sents = torch.LongTensor(sents).to(self.device)

            with torch.no_grad():
                probs = self.model.prob(progs, sents)

            idx = torch.argmax(probs).item()

            predicted_label = labels[idx]
            #print("predicated label is {}".format(predicted_label))

            return predicted_label
        else:
            return False

    def evaluate(self, table_name, sent):
        programs, masked_sent, mapping = self.parser.parse(table_name, sent)
        if programs:
            self.parser.rank(programs, masked_sent, mapping)
        else:
            return False
"""